...
bool Messenger::SetNodeNewShardGuardNetworkInfo(
  ...
  if (!result.data().IsInitialized()) {
    LOG_GENERAL(
        WARNING,
        "NodeLookupSetShardGuardNetworkInfoUpdate.Data initialization failed");
    return false;
  }
  ...
  if (!result.IsInitialized()) {
    LOG_GENERAL(
        WARNING,
        "NodeLookupSetShardGuardNetworkInfoUpdate initialization failed");
    return false;
  }
...
bool Messenger::GetNodeNewShardGuardNetworkInfo(const bytes& src,
  ...
  if (!Schnorr::Verify(tmp, 0, tmp.size(), signature, shardGuardPubkey)) {
    LOG_GENERAL(WARNING,
                "NodeLookupSetShardGuardNetworkInfoUpdate signature wrong");
    return false;
  }
